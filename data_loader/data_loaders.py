...
class CIFAR_data_loader(BaseDataLoader):
    def __init__(self, data_dir, batch_size=4, download=True, shuffle=True, validation_split=0.1, num_workers=0, flavor=10, training=True):
        preprocess = None
        if training:
            preprocess = CIFAR_TRAIN_PRE

        else:
            preprocess = CIFAR_TEST_PRE
...
class dist_CIFAR_data_loader():
    def __init__(self, data_dir, batch_size=4, validation_split=0.1, num_workers=0, pin_memory=True, flavor=100, training=True):
        preprocess = None
        if training:
            preprocess = CIFAR_TRAIN_PRE
        else:
        ...
        if flavor == 10:
            dataset = datasets.CIFAR10(
                data_dir, train=training, download=False, transform=preprocess)
        else:
